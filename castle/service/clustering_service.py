...
def _tree_to_dict(node) -> dict:
    ...
    d = {
        'name': node.name,
        'depth': node.depth,
        'n_frames': node.n_frames,
        'is_leaf': node.is_leaf,
    }
    if node.stop_reason:
        d['stop_reason'] = node.stop_reason
    if node.eps is not None:
        d['eps'] = node.eps
    if node.quality_score is not None:
        d['quality_score'] = round(node.quality_score, 4)
    if node.children:
...
class ClusteringSession:
    ...
    def auto_cluster(self, cluster_name: str = "init",
        ...
        """Run recursive hierarchical Behavior Microscope.
        
        Mirrors the manual multi-level workflow:
        1. Select cluster → UMAP → DBSCAN → get sub-clusters
        2. For each sub-cluster: if splittable → recurse, else → leaf
        3. Repeat until max_depth or stop conditions met
        
        UMAP config is auto-selected based on depth and cluster size,
        following Raiso's observed patterns from ctrl30 OFT project.
        
        Args:
            cluster_name: Starting cluster (default "init")
            max_depth: Maximum recursion depth (default 6, Raiso used up to 7)
            min_frames: Don't split clusters smaller than this (default 100)
            eps_values: DBSCAN eps values to try at each level
            progress_callback: Optional (message) callback for progress
            
        Returns:
            dict with tree structure, leaf count, and export paths
        """
        from castle.core.auto_cluster import (
            select_umap_config, find_best_eps, TreeNode,
            DEFAULT_EPS_VALUES,
        )
        ...
        device = getattr(self.latents, 'device', 'cpu')
        tree = TreeNode(name=cluster_name, depth=0, n_frames=len(self.latents.data),
                        is_leaf=False)
        
        # Queue of (cluster_name, depth, parent_tree_node) to process
        queue = [(cluster_name, 0, tree)]
        total_leaves = 0
        total_splits = 0
        
        while queue:
            cname, depth, parent_node = queue.pop(0)
            
            if progress_callback:
                progress_callback(f"Depth {depth}: splitting '{cname}' ...")
            
            logger.info(f"Auto-cluster depth={depth}: selecting '{cname}'")
            
            # Select the cluster
            self._current_cluster_name = cname
            self.local_latents = self.latents.select(selected_cluster=cname)
            n_frames = len(self.local_latents.data)
            
            if n_frames == 0:
                parent_node.is_leaf = True
                parent_node.stop_reason = "empty"
                parent_node.n_frames = 0
                total_leaves += 1
                continue
            
            # Check stop: too few frames
            if n_frames < min_frames:
                parent_node.is_leaf = True
                parent_node.stop_reason = "min_frames"
                parent_node.n_frames = n_frames
                total_leaves += 1
                continue
            
            # Check stop: max depth
            if depth >= max_depth:
                parent_node.is_leaf = True
                parent_node.stop_reason = "max_depth"
                parent_node.n_frames = n_frames
                total_leaves += 1
                continue
            
            # Select UMAP config based on depth and size
            umap_config = select_umap_config(depth, n_frames)
            
            # Run UMAP
            try:
                self.local_latents.build_embedding(umap_config)
            except Exception as e:
                logger.warning(f"UMAP failed for '{cname}': {e}")
                parent_node.is_leaf = True
                parent_node.stop_reason = f"umap_failed: {e}"
                parent_node.n_frames = n_frames
                total_leaves += 1
                continue
            
            # Find best eps via sweep
            embedding = self.local_latents.embedding
            valid_mask = np.ones(n_frames, dtype=bool)  # local data is already filtered
            dummy_full = np.zeros(n_frames, dtype=int)
            
            best = find_best_eps(embedding, dummy_full, valid_mask, eps_values, device)
            
            if best is None:
                # No valid clustering found → leaf
                parent_node.is_leaf = True
                parent_node.stop_reason = "no_valid_eps"
                parent_node.n_frames = n_frames
                total_leaves += 1
                continue
            ...
            if len(real_clusters) < 2:
                # Only one cluster → can't split → leaf
                parent_node.is_leaf = True
                parent_node.stop_reason = "single_cluster"
                parent_node.n_frames = n_frames
                total_leaves += 1
                continue
            
            # We have sub-clusters! Label them all and import
            parent_node.umap_config = umap_config
            parent_node.eps = best.eps
            parent_node.quality_score = best.quality_score
            parent_node.n_frames = n_frames
            parent_node.is_leaf = False
            total_splits += 1
            
            self.auto_label_all(parent_name=cname)
            
            # Import local clusters into global latent (without full submit)
            self.latents.import_local_latent(self.local_latents)
            
            # Determine which children to recurse on
            for cid in real_clusters:
                child_mask = self.local_latents.cluster == cid
                child_frames = int(child_mask.sum())
                
                # Get the name assigned to this sub-cluster
                if cid in self.local_latents.export:
                    child_name = self.local_latents.export[cid]['name']
                else:
                    continue
                ...
                child_node = TreeNode(
                    name=child_name,
                    depth=depth + 1,
                    n_frames=child_frames,
                    is_leaf=False,
                )
                parent_node.children.append(child_node)
                
                # Queue for further splitting
                queue.append((child_name, depth + 1, child_node))
            
            # Handle noise points (-1) — they become leaf
            noise_count = int((self.local_latents.cluster == -1).sum())
            if noise_count > 0:
                logger.info(f"  {noise_count} noise frames at depth {depth}")
        
        # Final submit to export CSVs
        submit_result = self.submit()
        
        # Count tree stats
        def count_tree(node):
            """Count leaves and max depth."""
            if node.is_leaf or not node.children:
                return 1, node.depth
            leaves = 0
            max_d = node.depth
            for child in node.children:
                cl, cd = count_tree(child)
                leaves += cl
                max_d = max(max_d, cd)
            return leaves, max_d
        
        n_leaves, max_d = count_tree(tree)
