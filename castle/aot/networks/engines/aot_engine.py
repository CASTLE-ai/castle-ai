...
class AOTEngine(nn.Module):
    ...
    def add_reference_frame(self,
                            ...
                            obj_nums=None,
                            img_embs=None):
        # print("Run add_reference_frame at aot engine")
        ...
        _, _, lstt_long_memories, lstt_short_memories = self.curr_lstt_output
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        # print('llm status', self.long_term_memories[0][0][:3,0,:3])
...
class AOTInferEngine(nn.Module):
    ...
    def match_propogate_one_frame(self, img=None, img_embs=None):
        reused_img_embs = img_embs
        for aot_engine in self.aot_engines:
            aot_engine.match_propogate_one_frame(img, img_embs=reused_img_embs)
            if reused_img_embs is None:  # reuse image embeddings
                reused_img_embs = aot_engine.curr_enc_embs
