...
| `CASTLE_GPU_DEVICE` | idlest GPU | str | Pins single-GPU ops to a specific card via `cuda:N`. Honored by SAM segmentation, cuML clustering, and distance helpers; when set it overrides automatic "idlest GPU" selection. |
...
| `CASTLE_MULTI_GPU_DETERMINISTIC` | off | bool | When multi-GPU is active, set truthy to force the slower per-GPU-reproducible cuDNN-deterministic path; unset keeps the fast cuDNN-benchmark + fp16 autocast path. |
| `PYTHONHASHSEED` | set from seed | str | Written with the master seed by `set_global_seed`; only affects child processes spawned afterwards. |
| `CUBLAS_WORKSPACE_CONFIG` | set under strict CUDA | str | Set to `:4096:8` (via `setdefault`) when strict CUDA is on, as required by deterministic cuBLAS. |
...
| Variable | Default | Type | Effect |
|----------|---------|------|--------|
| `CASTLE_USABLE_CPUS` | detected | int | Overrides the affinity/cgroup/`os.cpu_count()`-derived usable-core count that DataLoader worker counts and the extraction torch-thread cap size against. |
...
| `CASTLE_TORCH_THREADS` | auto | int | Overrides intra-op torch thread count during extraction; unset, capped to ≤8 on large/cgroup-limited hosts when extracting on CUDA (CPU extraction keeps torch defaults). |
| `CASTLE_PREPARE_LOADERS` | `4` | int | Cap on concurrent file-loader threads during Prepare. |
| `CASTLE_PREPARE_PREFETCH` | on | bool | Set `0`/`false`/`no` to disable Prepare's background prefetch (no load/GPU overlap). |
...
| `HDF5_USE_FILE_LOCKING` | set to `FALSE` | str | Set on `import castle` (via `setdefault`, so an explicit operator value wins) because HDF5 file locking hangs on network filesystems; `app.py` sets it to `FALSE` unconditionally. |
| `COLAB_GPU` | unset | presence | Detected by `app.py` (Google Colab): turns on `--share` and adds the Colab paths (`/content/drive/MyDrive/castle-projects`, `/tmp`, `/content`) to Gradio's `allowed_paths`. |
...
| `XFORMERS_DISABLED` | set automatically | str | Set to `1` (via `setdefault`) before loading DINOv2 on a non-CUDA device, because xformers has no CPU/MPS kernels. An explicit value is kept. |
| `CASTLE_ALLOW_UNVERIFIED_CKPT` | unset | str | Skips the DINOv3 checkpoint SHA-256 verification done before load. **Any non-empty value** (even `0`) skips it; set it only when intentionally using a custom (non-release) checkpoint. |
...
| `CASTLE_FAULTHANDLER` | on (off on Windows) | bool | Enables the stdlib faulthandler so `kill -USR1 <pid>` dumps all Python thread stacks (diagnose hangs without py-spy/root); set falsey to opt out. Off by default on Windows (no SIGUSR1 there, and the handler prints noisy first-chance access violations from native DLLs); set `1` to opt in. |
