...
def _ethogram_bar_data(bout_stats: dict):
    """Return ``(names, freqs_pct)`` for the ethogram frequency bar chart.

    Both lists are derived from ONE shared iteration sorted by *integer*
    cluster id, so ``names[i]`` and ``freqs_pct[i]`` always describe the same
    cluster. (Sorting by ``int`` — not string — also keeps ``"10"`` after
    ``"2"``.) Pure/CPU-only; extracted so the pairing can be unit-tested
    without rendering a figure.
    """
    items = sorted(bout_stats.items(), key=lambda x: int(x[0]))
    names = [bs.get("cluster_name", cid) for cid, bs in items]
    freqs = [float(bs.get("frequency", 0)) * 100 for _cid, bs in items]
    return names, freqs


def _provenance_html(env: dict) -> str:
    """Render a compact provenance line from a ``collect_run_environment()`` dict.

    The HTML report is the publication artifact, so it should state the exact
    stack that produced it (CASTLE + key library versions, device, GPU) — a
    reproduction can then tell a cuML-GPU run from an sklearn-CPU one.
    """
    pkgs = env.get("packages", {}) or {}
    key = [f"{name} {pkgs[name]}"
           for name in ("torch", "numpy", "scikit-learn", "umap-learn", "cuml")
           if pkgs.get(name)]
    head = [f"CASTLE {env.get('castle_version', '?')}",
            f"Python {env.get('python', '?')}",
            f"device={env.get('device', '?')}"]
    gpus = env.get("gpus") or []
    if gpus:
        head.append("GPU: " + ", ".join(gpus))
    return (
        "<div class='provenance' style='font-size:11px;color:#888;margin-top:6px'>"
        f"{' · '.join(head)}"
        + (f"<br/>{' · '.join(key)}" if key else "")
        + "</div>"
    )
...
class ReportGenerator:
    ...
    def _try_render_ethogram_plot(self, data: dict) -> str:
        ...
        try:
            ...
            colors = [color_for_cluster(i) for i in range(len(names))]
    ...
    def _try_render_quality_plot(self, data: dict) -> str:
        ...
        try:
            ...
            from castle.core.config import color_for_cluster
            colors = [color_for_cluster(i) for i in range(len(unique))]
    ...
    def _render_footer(self) -> str:
        ...
        provenance = _provenance_html(collect_run_environment())
        return (
            f"<div class='footer'>"
