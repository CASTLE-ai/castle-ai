...
def create_preprocess_ui(
    ...
    with gr.Column(visible=False) as ui["wrapper"]:
        ...
        # Read-only display of the auto-computed session name. Kept legible in
        # both light and dark themes by the global CASTLE_CSS read-only fix
        # (Gradio 6 otherwise leaves disabled single-line inputs with a white
        # background in dark mode -> white-on-white).
